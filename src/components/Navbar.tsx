...
export default function Navbar() {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => { if (otherRef.current && !otherRef.current.contains(e.target as Node)) setOtherOpen(false); };
  ...
  const renderAuthButtons = (mobile = false) => {
    if (!user) return (
      <button onClick={() => { setOpen(false); navigate('/auth'); }} className={cn('inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3.5 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700', mobile && 'w-full justify-center')}>
    ...
    return (
      <div className={cn('flex items-center gap-2', mobile && 'w-full flex-col')}>
        {permissions.size > 0 && (
          <button onClick={() => { setOpen(false); navigate('/admin/dashboard'); }} className={cn('inline-flex items-center gap-2 rounded-lg bg-brand-700 px-3.5 py-2 text-sm font-medium text-white hover:bg-brand-800', mobile && 'w-full justify-center')}>
            <LayoutDashboard className="h-4 w-4" /> Admin
          </button>
        )}
        <button onClick={handleLogout} className={cn('inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3.5 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700', mobile && 'w-full justify-center')}>
  ...
  const otherActive = [...kavlingNav, ...otherNav].some((n) => isActive(n.to));
    ...
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
        ...
        <div className="hidden items-center gap-1 lg:flex">
          {mainNav.map((item) => { const active = isActive(item.to); return (
          ...
          <div className="relative" ref={otherRef}>
            <button onClick={() => setOtherOpen((o) => !o)} className={cn('inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium transition', otherActive ? 'bg-brand-50 text-brand-700 dark:bg-brand-900/30 dark:text-brand-300' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800')}>
            ...
            {otherOpen && (
              <div className="absolute right-0 mt-1 w-48 rounded-lg border border-slate-200 bg-white py-1 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                ...
                {kavlingNav.map((item) => { const active = isActive(item.to); return (
                ...
                {otherNav.map((item) => { const active = isActive(item.to); return (
                  <Link key={item.to} to={item.to} onClick={() => setOtherOpen(false)} className={cn('flex items-center gap-2 px-3 py-2 text-sm font-medium transition', active ? 'bg-brand-50 text-brand-700 dark:bg-brand-900/30 dark:text-brand-300' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800')}>
                    <item.icon className="h-4 w-4" /> {item.label}
                  </Link>
                ); })}
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setOpen((o) => !o)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800 lg:hidden">
